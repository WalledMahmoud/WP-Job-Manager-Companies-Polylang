<?php
/**
 * Plugin Name: WP Job Manager - Company Profiles - Polylang Extend
 * Plugin URI:  https://github.com/WalledMahmoud/wp-job-manager-companies-polylang-Polylang
 * Description: Output a list of all companies that have posted a job, with a link to a company profile and adapted with Polylang to display the company names according to their jobs language
 * Author:      Walled Mahmoud Soliman
 * Author URI:  https://walledmahmoud.github.io/WalledMahmoud/
 * Version:     1.0
 * Text Domain: wp-job-manager-companies-polylang
 */

// Exit if accessed directly
if ( ! defined( 'ABSPATH' ) ) exit;

class polylang_Job_Manager_Companies {
	...
	private function setup_actions() {
		add_shortcode( 'job_manager_companies_polylang', array( $this, 'shortcode' ) );
	...
	/**
	 * Register the `[job_manager_companies_polylang]` shortcode.
	 *
	 * @since WP Job Manager - Company Profiles 1.0
	 *
	 * @param array $atts
	...
	public function build_company_archive( $atts ) {
		...
		/**
		* Here Is My Idea In Few Steps :-
			* Create Query To Get The Posts Inside Specific Post Type
			* Assign The Language Of These Posts To The Current Language To Display
			* Loop The Posts And Store Their Company Names To Go Through
			** Bingo !!
		*/
		$args = array (
		...
		if ( $jobs ) {
			...
			foreach($jobs as $job) {
				$companies[] = get_post_meta($job->ID, '_company_name', true);
	...
	function page_title($title) {
		...
		$title = sprintf( __( 'Jobs at %s', 'wp-job-manager-companies-polylang' ), $company ) . " $sep $title";
	...
	public function load_plugin_textdomain() {
		$locale = apply_filters( 'plugin_locale', get_locale(), 'wp-job-manager-companies-polylang' );

		load_textdomain( 'wp-job-manager-companies-polylang', WP_LANG_DIR . "/wp-job-manager-companies-polylang/wp-job-manager-companies-polylang-$locale.mo" );
		load_plugin_textdomain( 'wp-job-manager-companies-polylang', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	}
}
add_action( 'plugins_loaded', array( 'polylang_Job_Manager_Companies', 'instance' ) );
